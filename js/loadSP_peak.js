...
function loadGiayNike() {
    ...
    <div class="chon-size">
        ...
        document.getElementById("sanpham").innerHTML += sp1;
        pushArr(sp1);
    });
}
...
function loadGiayAdidas() {
    ...
    <div class="chon-size">
        ...
        document.getElementById("sanpham").innerHTML += sp2;
        pushArr(sp2);
    });
}
...
function loadGiayTE() {
    ...
    sanpham.GiayBongRo[0].Giay_Tre_Em.forEach(sanpham => {
        var sizeHTML = "";
        // sanpham.size.forEach(size => {
        //   sizeHTML += `<button onclick="chonsize(this)">${size}</button>`;
        // });
      ...
      <div class="chon-size">
        ...
        document.getElementById("sanpham").innerHTML += sp3;
        pushArr(sp3);
    });
...
//load giày NBA
function loadGiayNBA() {
    sanpham.GiayBongRo[0].Giay_NBA.forEach(sanpham => {
        var sizeHTML = "";
        // sanpham.size.forEach(size => {
        //   sizeHTML += `<button onclick="chonsize(this)">${size}</button>`;
        // });
        ...
        document.getElementById("sanpham").innerHTML += sp4;
        pushArr(sp4);
    });
}

//load giayPeak
function loadGiayPeak() {
    sanpham.GiayBongRo[0].Giay_Peak.forEach(sanpham => {
        var sizeHTML = "";
        // sanpham.size.forEach(size => {
        //   sizeHTML += `<button onclick="chonsize(this)">${size}</button>`;
        // });
        ...
        document.getElementById("sanpham").innerHTML += sp5;
        pushArr(sp5);
    });
}
...
function loadGiayAnTa() {
    sanpham.GiayBongRo[0].Giay_AnTa.forEach(sanpham => {
        ...
        document.getElementById("sanpham").innerHTML += sp6;
        pushArr(sp6);
    });
}
//tìm giá min, max
...
function MinMax() {
    ...
    Gmin = timGiaMin(sanpham.GiayBongRo[0].Giay_Peak);
    // Tìm giá trị lớn nhất trong ArrMax
    Gmax = timGiaMax(sanpham.GiayBongRo[0].Giay_Peak);
}
...
function intsertGiay() {
    allProducts.splice(0, allProducts.length);
        for (var id = 0; id < sanpham.GiayBongRo[0].Giay_Peak.length; id++) {
            var productName = sanpham.GiayBongRo[0].Giay_Peak[id];
            if (productName) {
...
function showProducts(page) {
    ...
    if(sx=="Tang"){filteredProducts.sort(function (a, b) {
        return a.giaM - b.giaM;
    });}
    if(sx=="Giam"){filteredProducts.sort(function (a, b) {
...
function SapXepTang(min, max) {
    ...
    sxloai_tang(sanpham.GiayBongRo[0].Giay_Peak, min, max) 
}
...
function SapXepGiam(min, max) {
 ...
 sxloai_giam(sanpham.GiayBongRo[0].Giay_Peak, min, max) 
