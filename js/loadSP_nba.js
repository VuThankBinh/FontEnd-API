...
function MinMax() {
    ...
    Gmin = timGiaMin(sanpham.GiayBongRo[0].Giay_NBA);
    // Tìm giá trị lớn nhất trong ArrMax
    Gmax = timGiaMax(sanpham.GiayBongRo[0].Giay_NBA);
}
...
function intsertGiay() {
    ...
    for (var id = 0; id < sanpham.GiayBongRo[0].Giay_NBA.length; id++) {
        var productName = sanpham.GiayBongRo[0].Giay_NBA[id];
        if (productName) {
...
function SapXepTang(min, max) {
    ...
    sxloai_tang(sanpham.GiayBongRo[0].Giay_NBA, min, max)
}
...
function SapXepGiam(min, max) {
    ...
    sxloai_giam(sanpham.GiayBongRo[0].Giay_NBA, min, max)
